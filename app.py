...
# Clave de sesión Flask (en Railway: QR_SECRET_KEY)
...
# URL base pública (para mostrar enlaces)
  ...
  <div class="container" style="max-width: 900px;">
      ...
      <div class="list-group mt-3">
        {% for qr in qrs %}
          <div class="list-group-item d-flex justify-content-between align-items-center">
            <div class="me-3">
              <div class="fw-semibold">{{ qr.qr_code_string or ('ID ' ~ qr.id) }}</div>
              <small class="text-muted">QR ID: {{ qr.id }}</small>
              {% if qr.public_code %}
                <div><small>Link público: <a target="_blank" href="{{ public_base }}/v/{{ qr.public_code }}">{{ public_base }}/v/{{ qr.public_code }}</a></small></div>
              {% endif %}
            </div>
            <div class="d-flex gap-2">
              <a class="btn btn-sm btn-primary" target="_blank" href="{{ url_for('emergencia', codigo_id=qr.id) }}">Ver ficha</a>
            </div>
          </div>
        {% endfor %}
      </div>
    {% else %}
      <div class="alert alert-info mt-4">
        Todavía no tenés QR asociados. Podés reclamar una etiqueta escaneando su QR o visitando su link público.
      </div>
    {% endif %}
  </div>
</body>
...
def panel():
    conn = get_conn()
    cur = conn.cursor(dictionary=True)
    cur.execute("SELECT id, qr_code_string, public_code FROM qr_codes WHERE user_id=%s ORDER BY id", (g.user["id"],))
    qrs = cur.fetchall()
    cur.close(); conn.close()
    return render_template_string(PANEL_HTML, qrs=qrs, user_email=g.user["email"], public_base=PUBLIC_BASE)

# =========================
# Vista pública de emergencia (por ID numérico)
# =========================
...
# =========================
# NUEVO: vista pública por public_code
# =========================
UNCLAIMED_HTML = """
<!doctype html>
<html lang="es">
...
<body class="bg-light">
  <div class="container" style="max-width: 640px;">
    <div class="card shadow mt-5">
      <div class="card-body p-4">
        <h3 class="mb-3">Etiqueta no reclamada</h3>
        <p class="text-muted">Código: <code>{{ code }}</code></p>
        <p>Esta etiqueta todavía no pertenece a ningún usuario.</p>
        <ol class="mb-3">
          <li>Iniciá sesión con tu cuenta.</li>
          <li>Reclamá esta etiqueta para asociarla a tu perfil.</li>
        </ol>
        {% if logged_in %}
          <a class="btn btn-primary" href="{{ url_for('claim_code', public_code=code) }}">Reclamar ahora</a>
        {% else %}
          <a class="btn btn-primary" href="{{ url_for('login', next=url_for('claim_code', public_code=code)) }}">Iniciar sesión para reclamar</a>
        {% endif %}
      </div>
    </div>
  </div>
...
@app.get("/v/<public_code>")
def view_by_public_code(public_code):
    """
    Si la etiqueta está reclamada → muestra la misma ficha que /emergencia/<id>.
    Si no está reclamada → muestra pantalla para reclamar.
    """
    try:
        conn = get_conn()
        cur = conn.cursor(dictionary=True)
        cur.execute("SELECT id, user_id FROM qr_codes WHERE public_code=%s", (public_code,))
        row = cur.fetchone()
        if not row:
            cur.close(); conn.close()
            return "Código inválido o inexistente.", 404

        if row["user_id"]:
            # Ya reclamada → mostrar ficha usando el ID
            qr_id = row["id"]
            cur.execute("""
                SELECT ed.nombre, ed.apellido, ed.telefono_1, ed.telefono_2,
                       ed.factor_sanguineo, ed.tiene_alergias, ed.instructivo_url
            ...
            if not data:
                return "No hay ficha asociada a este código aún.", 404

            # Reutilizamos la misma plantilla de emergencia
            html_template = '''
            <!DOCTYPE html>
            <html lang="es">
            <head>
            ...
            <body>
                ...
                <script>
                    ...
                    function mostrarNumeros(pais, region) {
                        ...
                        try {
                            const infoPais = emergencyNumbers[pais];
                            if (region && infoPais[region]) { numeros = infoPais[region]; }
                            else { numeros = infoPais.default; }
                        } catch { numeros = { "policía":"911", "bomberos":"100", "ambulancia":"107" }; }
                        document.getElementById("ubicacion").innerHTML =
                          `📍 ${region || pais}<br>Policía / Police: ${numeros.policía} · Bomberos / Fire: ${numeros.bomberos} · Ambulancia / Ambulance: ${numeros.ambulancia}`;
                    }
                    function obtenerProvincia(lat, lon) {
                        fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lon}`)
                          .then(r => r.json())
                          .then(data => { const pais = data?.address?.country || "Argentina"; const region = data?.address?.state || null; mostrarNumeros(pais, region); })
                          .catch(() => { document.getElementById("ubicacion").innerText = "No se pudo determinar la ubicación / Could not determine location"; });
                    }
                    if (navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(
                            pos => obtenerProvincia(pos.coords.latitude, pos.coords.longitude),
        ...
        # No reclamada → mostrar página de “Reclamar”
        logged_in = bool(g.user)
        cur.close(); conn.close()
        return render_template_string(UNCLAIMED_HTML, code=public_code, logged_in=logged_in)

    except Exception as e:
        return f"Error: {e}", 500

# =========================
# NUEVO: claim de etiqueta
# =========================
@app.route("/claim/<public_code>", methods=["GET", "POST"])
@login_required
def claim_code(public_code):
    # GET: confirmación simple → POST
    if request.method == "GET":
        html = """
        <!doctype html><html lang="es"><head>
          <meta charset="utf-8"><title>Confirmar reclamo</title>
          <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
        </head><body class="bg-light">
        <div class="container" style="max-width:640px;">
          <div class="card shadow mt-5"><div class="card-body p-4">
            <h3 class="mb-3">Reclamar etiqueta</h3>
            <p class="text-muted">Código: <code>{{ code }}</code></p>
            <p>Esta acción asociará la etiqueta a tu cuenta (<strong>{{ user_email }}</strong>).</p>
            <form method="post">
              <button class="btn btn-primary">Reclamar ahora</button>
              <a class="btn btn-link" href="{{ url_for('panel') }}">Cancelar</a>
            </form>
          </div></div>
        </div></body></html>
        """
        return render_template_string(html, code=public_code, user_email=g.user["email"])

    # POST: reclamar de forma atómica si está libre
    try:
        conn = get_conn()
        cur = conn.cursor()

        # 1) Verificar que exista y esté libre
        ...
        # 2) Reclamar (solo si user_id IS NULL)
        cur.execute(
            "UPDATE qr_codes SET user_id=%s, claimed_at=NOW() WHERE id=%s AND user_id IS NULL",
            (g.user["id"], qr_id)
        )
        # Si filas afectadas = 0, alguien la tomó justo antes
        if cur.rowcount == 0:
            cur.close(); conn.close()
            return "La etiqueta ya fue reclamada por otro usuario.", 409

        # 3) Asegurar que el usuario tenga emergency_data
        cur.execute("SELECT 1 FROM emergency_data WHERE user_id=%s LIMIT 1", (g.user["id"],))
        has = cur.fetchone()
        if not has:
            cur.execute("""
                INSERT INTO emergency_data
                (user_id, nombre, apellido, telefono_1, telefono_2, factor_sanguineo, tiene_alergias, instructivo_url)
                VALUES (%s, 'Nombre', 'Apellido', '1122334455', '1199887766', '0+', 0,
                        'https://www.argentina.gob.ar/salud/primeros-auxilios')
            """, (g.user["id"],))
        ...
        return f"Error al reclamar: {e}", 500
